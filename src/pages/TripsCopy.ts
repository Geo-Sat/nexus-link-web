...
const mockVehicles: Vehicle[] = [
  ...
  },
  {
    ...
    model: 'Canter',
    year: 2020,
    coordinates: [36.8324, -1.2985],
    heading: 0,
    speed: 0,
    status: 'inactive',
    lastUpdate: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000),
    isSelected: false,
    routeHistory: []
  },
  {
...
const mockHistoricalRoutes: Record<string, RoutePoint[]> = {
  '1': [
    {
      coordinates: [36.8219, -1.2921],
      timestamp: new Date(Date.now() - 4 * 60 * 60 * 1000),
...
export function TripsPage() {
  ...
  const handleSelectTrip = (trip: Trip) => {
    ...
    if (mockHistoricalRoutes[trip.vehicleId]) {
      setSelectedTrip({
        ...trip,
        route: mockHistoricalRoutes[trip.vehicleId]
      });
    }
  };
  ...
  const handleSearchHistorical = () => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      const historicalData = mockHistoricalRoutes[selectedVehicle.id] || [];
      setSelectedTrip({
        id: 'historical',
